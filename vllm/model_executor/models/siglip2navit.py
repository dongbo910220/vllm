...
def apply_rotary_pos_emb(
    ...
    q_embed = apply_rotary_emb_func(
        q.float(), cos.float(), sin.float(), interleaved
    ).type_as(q)
    k_embed = apply_rotary_emb_func(
        k.float(), cos.float(), sin.float(), interleaved
    ).type_as(k)
...
class Siglip2Attention(nn.Module):
    ...
    def forward(
        ...
        if self.use_rope:
            ...
            # SigLIP variants may use interleaved rotary layout; enable it
            # to better match HF behavior under 2D-RoPE.
...
class Siglip2Encoder(nn.Module):
    ...
    def __init__(
        ...
        # Disable windowing when explicitly turned off (<=0) or when
        # the derived ViT merger window size is effectively identity (==1).
        if isinstance(self.window_size, int) and self.window_size > 0:
            vit_ws = self.window_size // self.hidden_stride // self.patch_size
        else:
            vit_ws = 0
        self._vit_merger_window_size = vit_ws
    ...
    def get_window_index(self, grid_thw):
        ...
        vit_merger_window_size = self._vit_merger_window_size
        ...
        for grid_t, grid_h, grid_w in grid_thw:
            ...
            # Fast path: no windowing, identity order
            if self._disable_window:
                index = torch.arange(grid_t * llm_grid_h * llm_grid_w)
                window_index.append(index + window_index_id)
                cu_window_seqlens.append(
                    cu_window_seqlens[-1]
                    + (grid_t * llm_grid_h * llm_grid_w * self.spatial_merge_unit)
                )
    ...
    def forward(
        ...
        # Duplicate to cover the full 2D rotary span across head dims.
        ...
        reverse_indices = (
            torch.argsort(window_index) if not self._disable_window else None
        )
...
class Siglip2VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # PaddleOCR-VL projector applies a per-patch LayerNorm internally.
        # To better match HF behavior, allow skipping this post-encoder LN
        # so the projector receives pre-LN features. Controlled via env var
        # for diagnosis and parity checks, defaulting to keep LN disabled.
        import os

        if os.getenv("PADDLEOCRVL_SKIP_VISION_POST_LN", "1") != "0":
