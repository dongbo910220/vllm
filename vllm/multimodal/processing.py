...
from vllm.utils.jsontree import JSONTree
...
def _prompt_debug_enabled() -> bool:
    """Whether to emit prompt update debug artifacts.

    Controlled by env var PADDLEOCRVL_DEBUG_PROMPT=1.
    """
    try:
        return os.getenv("PADDLEOCRVL_DEBUG_PROMPT", "").strip() not in (
            "",
            "0",
            "false",
            "False",
...
def _prompt_debug_save(obj: dict[str, object], name: str) -> None:
    if not _prompt_debug_enabled():
        return
    try:
        out_dir = os.path.join("test_result", "prompt_debug")
        os.makedirs(out_dir, exist_ok=True)
        run_id = os.getenv("PADDLEOCRVL_RUN_ID") or time.strftime("%Y%m%d-%H%M%S")
        global _PROMPT_DEBUG_SEQ
        _PROMPT_DEBUG_SEQ += 1
        path = os.path.join(
            out_dir, f"vllm_{run_id}_{_PROMPT_DEBUG_SEQ:04d}_{name}.json"
        )
        with open(path, "w", encoding="utf-8") as f:
            json.dump(obj, f, ensure_ascii=False, indent=2)
    except Exception:
        # Never fail the main path due to debug saving
        pass
...
class InputProcessingContext:
    ...
    def _postprocess_output(
        ...
        """
        Post-process HF processor outputs to match model dtype, while
        preserving raw pixel tensors for PaddleOCR-VL to ensure parity with HF.

        - Default behavior: cast floating tensors to `self.model_config.dtype`.
        - Special-case: when the model type is `paddleocr_vl`, do NOT cast any
          tensor under the key `pixel_values` (leave as produced by HF).
        """

        # Detect if current model is PaddleOCR-VL.
        is_paddleocr_vl = False
        try:
            hf_conf = self.model_config.hf_config
            is_paddleocr_vl = getattr(hf_conf, "model_type", "") == "paddleocr_vl"
        except Exception:
            is_paddleocr_vl = False

        def _process(value: object, path: tuple[str, ...] = ()) -> object:
            # Recurse dict/list/tuple to retain key context (e.g. 'pixel_values').
            if isinstance(value, dict):
                return {k: _process(v, path + (k,)) for k, v in value.items()}
            if isinstance(value, list):
                return [_process(v, path) for v in value]
            if isinstance(value, tuple):
                return tuple(_process(v, path) for v in value)

            if isinstance(value, torch.Tensor):  # noqa: SIM102
                # This mimics the behavior of transformers.BatchFeature
                if value.is_floating_point():
                    # Skip dtype cast for raw image pixels in PaddleOCR-VL.
                    if is_paddleocr_vl and ("pixel_values" in path):
                        return value
                    return value.to(dtype=self.model_config.dtype)
...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    ...
    def _apply_prompt_updates(
        ...
        if _prompt_debug_enabled():
            # Summarize intended updates for debug
            dbg_updates: dict[str, list[dict[str, object]]] = {}
            try:
                for modality, per_item in mm_prompt_updates.items():
                    items_dbg: list[dict[str, object]] = []
                    for cand_list in per_item:
                        # record all candidate replacement content lengths
                        cand_dbg: list[dict[str, object]] = []
                        for upd in cand_list:
                            try:
                                content = upd.content.full
                                toks = _seq2tokens(tokenizer, content)
                                cand_dbg.append(
                                    {
                                        "mode": upd.mode,
                                        "content_len": len(toks),
                                    }
                                )
                            except Exception:
                                cand_dbg.append({"mode": upd.mode, "content_len": None})
                        items_dbg.append({"candidates": cand_dbg})
                    dbg_updates[modality] = items_dbg
            except Exception:
                pass
        ...
        if _prompt_debug_enabled():
            try:
                dbg: dict[str, object] = {
                    "before_token_ids": list(token_ids),
                    "after_token_ids": list(new_token_ids),
                    "used_text_fallback": used_text_fallback,
                    "match_result": {k: list(v) for k, v in match_result.items()},
                    # matched_updates intentionally omitted
                    # (redundant with match_result)
                    "mm_prompt_updates_summary": dbg_updates,
                    "placeholders": {
                        m: [
                            {
                                "offset": int(ph.to_range().offset),
                                "length": int(ph.to_range().length),
                                "num_embeds": int(ph.to_range().get_num_embeds()),
                            }
                            for ph in lst
                        ]
                        for m, lst in placeholders.items()
                    },
                }
                _prompt_debug_save(dbg, "apply_prompt_updates")
            except Exception:
                pass
                ...
                try:
                    dbg = {
                        ...
                        "placeholders": {
                            ...
                            for m, lst in mm_placeholders.items()
                        },
                    }
                    _prompt_debug_save(dbg, "maybe_apply_prompt_updates")
                except Exception:
                    pass
        else:
