...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        if self.uses_mrope:
            ...
            # Optional debug: persist the CPU slice we just copied so we can
            # verify non-zero positions for the scheduled prompt tokens.
            try:
                import os as _os

                if _os.getenv("PADDLEOCRVL_DEBUG_MROPE", "0") != "0":
                    run_id = _os.getenv("PADDLEOCRVL_RUN_ID", "run")
                    ...
                    _torch.save(
                        ...
                        _os.path.join(
                            out_dir, f"vllm_{run_id}_mrope_positions_step.pt"
                        ),
                    )
            except Exception:
                pass
    ...
    def _execute_mm_encoder(self, scheduler_output: "SchedulerOutput"):
        ...
        for (mm_hash, pos_info), output in zip(mm_hashes_pos, encoder_outputs):
            # Optional debug dump for PaddleOCR-VL multi-image diagnostics
            if os.getenv("PADDLEOCRVL_DEBUG_EMBED", "0") not in (
                "",
                "0",
                "false",
                "False",
                ...
                try:
                    ...
                    out_dir = os.path.join("test_result", branch)
                    os.makedirs(out_dir, exist_ok=True)
                    info = {
                        "mm_hash": str(mm_hash),
                        "pos_offset": int(getattr(pos_info, "offset", 0)),
                        "pos_length": int(getattr(pos_info, "length", 0)),
                        "is_embed_true": None
                        if pos_info.is_embed is None
                        else int(pos_info.is_embed.sum().item()),
                    }
                    path = os.path.join(out_dir, "embed_positions.jsonl")
