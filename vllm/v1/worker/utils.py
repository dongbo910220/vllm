...
def scatter_mm_placeholders(
    ...
    # Align mask width with actual embeds length for robustness. This guards
    # against rare width mismatches introduced by upstream processors.
    num_true = int(is_embed.sum().item())
    num_embeds = int(embeds.shape[0])
    if num_true != num_embeds:
        # Only attempt adjustment when explicitly enabled or for known
        # PaddleOCR‑VL style prompts where placeholders are contiguous.
        adjust_ok = os.getenv("PADDLEOCRVL_ADJUST_MASK", "1") not in (
            "0",
            "false",
            "False",
        )
        if adjust_ok and num_embeds <= int(is_embed.shape[0]):
            # Expand or shrink the contiguous True region around existing True
            idx = torch.nonzero(is_embed, as_tuple=False).flatten()
            if idx.numel() == 0:
                # If no True yet, start from the center region.
                center = is_embed.shape[0] // 2
                start = max(0, center - (num_embeds // 2))
                end = min(is_embed.shape[0], start + num_embeds)
                is_embed = torch.zeros_like(is_embed, dtype=torch.bool)
                is_embed[start:end] = True
            else:
                left = int(idx[0].item())
                right = int(idx[-1].item())
                curr = right - left + 1
                target = num_embeds
                # Prefer expanding to the right, then to the left
                if target > curr:
                    need = target - curr
                    # Right expansion
                    r_extra = min(need, int(is_embed.shape[0]) - 1 - right)
                    if r_extra > 0:
                        is_embed[right + 1 : right + 1 + r_extra] = True
                    need -= r_extra
                    # Left expansion if needed
                    l_extra = min(need, left)
                    if l_extra > 0:
                        is_embed[left - l_extra : left] = True
                elif target < curr:
                    # Shrink by turning off tail positions first
                    over = curr - target
                    if over > 0:
                        cut = min(over, right - left + 1)
                        is_embed[right - cut + 1 : right + 1] = False
        # Recompute post-adjustment
        num_true = int(is_embed.sum().item())
        if num_true != num_embeds:
            msg = (
                "shape mismatch after mask adjust: "
                f"embeds={num_embeds}, mask_true={num_true}"
            )
