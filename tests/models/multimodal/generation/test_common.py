...
VLM_TEST_SETTINGS = {
    ...
    # PaddleOCR-VL: integrate into common VLM strict tests (image + multi-image)
    # Use a neutral USER/ASSISTANT wrapper and the model's native image placeholders.
    "paddleocr_vl": VLMTestInfo(
        ...
        comparator=check_outputs_equal,
        # Restrict comparison to (ids, text) for strict equality
        vllm_output_post_proc=lambda vllm_output, model: vllm_output[:2],
        hf_output_post_proc=lambda hf_output, model: hf_output[:2],
        # Default: keep (0.25, 0.25) disabled; enable via env for focused debug
        image_size_factors=(
            [(), (0.25,)]
            if os.getenv("PADDLEOCRVL_TEST_025025", "0") in ("", "0", "false", "False")
            else [(), (0.25,), (0.25, 0.25)]
        ),
